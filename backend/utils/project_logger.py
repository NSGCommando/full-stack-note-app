...
def get_project_logger(module_name:str,level:int=logging.INFO,log_dir:Path|None=None)->logging.Logger:
    """
    Returns a logger that logs to both console and a file named after the caller module name.
    ...
    if not logger.handlers:  # Prevent adding multiple handlers if imported multiple times 
        # (hasHandlers() checks parent handlers as well; instead use handlers prop for only this logger's handlers)
