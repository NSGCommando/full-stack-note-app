...
test_logger = get_project_logger(module_name="testing.backend.test_API",level=logging.DEBUG,log_dir=Path(__file__).resolve().parent)
...
def assertion_wrapper(response,expected_status:int,test_func):
    """Helper to wrap asserts and log test JSON responses"""
    test_logger.info(f"{test_func.__name__} Response: {response.json()}")
...
def test_signup(session_manager,gen_good_data):
    ...
    assertion_wrapper(signup_check_username,200,test_signup)
    ...
    assertion_wrapper(signup_request,201,test_signup)
...
def test_login(session_manager,gen_good_data):
    ...
    assertion_wrapper(login_request,200,test_login)
...
def test_delete(login_setup):
    ...
    assertion_wrapper(delete_request,403,test_delete)
    session.get(f"{API_URL}/api/logout")
...
def test_logout(login_setup):
    ...
    assertion_wrapper(logout_request,200,test_logout)
...
def test_empty_data(session_manager,gen_good_data):
    ...
    assertion_wrapper(signup_t1,400,test_empty_data)
    signup_t2 = session.post(f"{API_URL}/api/signup",json=signup_no_password)
    assertion_wrapper(signup_t2,400,test_empty_data)
...
def test_bad_data_username(session_manager,gen_bad_data):
    ...
    assertion_wrapper(bad_signup_check_username,400,test_bad_data_username) # reject bad username
...
def test_bad_data_password(session_manager,gen_bad_data):
    ...
    assertion_wrapper(bad_signup_check_password,200,test_bad_data_password) # accept valid username
    ...
    assertion_wrapper(bad_signup_request_password,400,test_bad_data_password) # reject bad password
...
def test_malicious_attacker_fake_header(session_manager,gen_good_data):
    ...
    assertion_wrapper(signup_request_hacker,403,test_malicious_attacker_fake_header)
...
def test_malicious_attacker_unsafe_name(session_manager,gen_good_data):
    ...
    signup_data_hacker = user_data_hacker
    signup_request_hacker = session.post(f"{API_URL}/api/signup",json=signup_data_hacker)
    assertion_wrapper(signup_request_hacker,400,test_malicious_attacker_unsafe_name)
...
def test_malicious_attacker_invalid_jwt(login_setup):
    ...
    assertion_wrapper(access_invalid_request,401,test_malicious_attacker_invalid_jwt)
